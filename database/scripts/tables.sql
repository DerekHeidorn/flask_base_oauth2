...
CREATE TABLE public.TB_FRIENDSHIP_STA_CD
(
  FRIENDSHIP_STA_CD character(1) NOT NULL,
  FRIENDSHIP_STA_DE character varying(30) NOT NULL,
  CONSTRAINT PK_FRIENDSHIP_STA_CD PRIMARY KEY (FRIENDSHIP_STA_CD)
)
WITH (
  OIDS=FALSE
);
...
ALTER TABLE public.TB_USER_STA_CD OWNER TO postgres;
...
COMMENT ON COLUMN public.TB_USER.USER_ALIAS IS 'Unique Alias for a User';
...
-- Table: public.TB_FRIENDSHIP

CREATE TABLE public.TB_FRIENDSHIP
(
  USER_ID integer NOT NULL, -- System-generated ID for a User.
  FRIEND_USER_ID integer NOT NULL, -- Security Group Surrogate Key
  FRIENDSHIP_STA_CD character(1) NOT NULL DEFAULT 'P', --Pending
  FRIENDSHIP_FROM_TS timestamp without time zone NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT XPKTB_FRIENDSHIP PRIMARY KEY (USER_ID, FRIEND_USER_ID),
  CONSTRAINT FRIENDSHIP_TO_USER FOREIGN KEY (USER_ID)
      REFERENCES public.TB_USER (USER_ID) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT FRIENDSHIP_TO_FRIEND_USER FOREIGN KEY (FRIEND_USER_ID)
      REFERENCES public.TB_USER (USER_ID) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
   CONSTRAINT FRIENDSHIP_TO_FRIENDSHIP_STA_CD FOREIGN KEY (FRIENDSHIP_STA_CD) REFERENCES TB_FRIENDSHIP_STA_CD (FRIENDSHIP_STA_CD)
)
WITH (
  OIDS=FALSE
);
...
CREATE INDEX XIF1TB_FRIENDSHIP
  ON public.TB_FRIENDSHIP
  USING btree
  (USER_ID);

CREATE INDEX XIF2TB_FRIENDSHIP
  ON public.TB_FRIENDSHIP
  USING btree
  (FRIEND_USER_ID);
...
-- Table: public.TB_FRIENDSHIP_HISTORY

CREATE TABLE public.TB_FRIENDSHIP_HISTORY
(
  FRIENDSHIP_HISTORY_ID serial NOT NULL, -- System-generated ID for a User.
  USER_ID integer NOT NULL,
  FRIEND_USER_ID integer NOT NULL,
  FRIENDSHIP_STA_CD character(1) NOT NULL DEFAULT 'P', --Pending
  FRIENDSHIP_FROM_TS timestamp without time zone NOT NULL,
  FRIENDSHIP_TO_TS timestamp without time zone NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT XPKTB_FRIENDSHIP_HISTORY PRIMARY KEY (FRIENDSHIP_HISTORY_ID),
  CONSTRAINT FRIENDSHIP_HISTORY_TO_USER FOREIGN KEY (USER_ID)
      REFERENCES public.TB_USER (USER_ID) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT FRIENDSHIP_HISTORY_TO_FRIEND_USER FOREIGN KEY (FRIEND_USER_ID)
      REFERENCES public.TB_USER (USER_ID) MATCH SIMPLE
      ON UPDATE NO ACTION ON DELETE NO ACTION
)
WITH (
...
CREATE INDEX XIF1TB_FRIENDSHIP_HISTORY
  ON public.TB_FRIENDSHIP
  USING btree
  (USER_ID);

CREATE INDEX XIF2TB_FRIENDSHIP_HISTORY
  ON public.TB_FRIENDSHIP
  USING btree
  (FRIEND_USER_ID);
